...
echo "Students are not allowed to browse teaching contents";
...
$sql="SELECT * FROM $tbl_name WHERE access='public' AND articletype='teachingar'";
...
echo "Private Articles Created By Me";
?>
...
mysql_connect("$host", "$username", "$password")or die("cannot connect");
mysql_select_db("$db_name")or die("cannot select DB");
$sql="SELECT * FROM $tbl_name WHERE access='private' AND articletype='teachingar' AND userid=$ui";
$result=mysql_query($sql);
$count=mysql_num_rows($result);
